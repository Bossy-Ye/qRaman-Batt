...
2. Take **P95(|Δν|,ₖ)** = 95th percentile of |Δνₖ|.  
...
> To start this project, we set specifically chosen value for each band.
...
- Sweep the decision threshold and plot validation **FAR/FRR and F1(PEAK) vs threshold**.  
- Pick **τ** where false accepts (bad windows marked “peak”) stay below our target (e.g., FAR ≤ 0.5%) while recall on peaks remains acceptable.  
...
- Typical starting point: **κ_min ≈ 0.60** with target **AUROC ≥ 0.90**. Start with 0.60, then re-fit it from our local data and pick **κ_min** that hits **AUROC ≥ 0.90**.
...
|-----------|-----------------------------------------------------|----------------------------------------|
| `tol_k`   | `max(5×res, P95(Δν,k) + m_chem)`             | From local + DOE                        |
